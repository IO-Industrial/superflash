...
#define EP_IN 0x80
        ...
        public:
            ...
            bool open_device()
            {
                ...
                _handle.set_handle(h);

                _handle.get_configuration(&config);
                SPDLOG_TRACE("bConfigurationValue = 0x{:x}", config);

                if (_handle.is_driver_active(0))
                {
                    _handle.detach_driver(0);
                }

                if (!_handle.claim_interface(0))
                {
            ...
            int transfer_hid(int report, uint8_t *data, unsigned int length, int *last, bool use_ep1, int max_transfer)
            {
                ...
                if (report < 3)
                {
                    ...
                    {
                        *last = 0;
                        err = _handle.interrupt_transfer(1,
                ...
                else
                {
                    ...
                    *last = 0;
                    ...
                    err = _handle.interrupt_transfer(1 + EP_IN, tmp, length + 1, last, timeout);
